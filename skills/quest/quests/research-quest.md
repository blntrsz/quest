...
1. **Query the wiki first.** Load the **wiki** ability, and run its **Query** operation over the ticket's question. Report what the wiki already holds, and build on it rather than researching it again. Done when the wiki's answer is read, or the wiki is known to hold nothing on the question.
...
3. **Draft the findings as a Markdown comment on the ticket**, citing each claim's source. Load and run the **writing** ability over the draft, so it picks the right mode and reads on the first pass.
...
8. **Ingest the findings into the wiki.** Load the **wiki** ability, and run its **Ingest** operation, so the findings become wiki pages that cite the ticket as their reference. Done when the wiki holds them and the change is committed.
