...
Load one or more runbooks with `scripts/load.sh quest <name> [<name>...]` from the repo root. List them with `scripts/load.sh list quest`. Loading a runbook does not run the quest.
...
2. **Resolve its runbook.** Read the `quest:<name>` label. If the ticket carries no quest label, stop and say so. Otherwise run `scripts/load.sh quest <name>` from the repo root. The script prints one `<quest name="...">` element whose text is that runbook. If the script exits non-zero, it names the miss and lists the quest names that exist. Stop and say so. Never improvise a runbook.
...
1. **Settle the type.** It comes from the user. If none fits, run `scripts/load.sh list quest` from the repo root and ask. A missing type means a missing runbook, which the human writes. You file tickets, not runbooks.
