...
3. **Add the runbook.** Create `skills/quest/quests/<type>-quest.md` using `skills/create-quest-type/TEMPLATE.md` as a starting point. Set frontmatter `name` to the bare type name and write a one-line description of the outcome and when to use the type. Include an `abilities` list only when the workflow needs those abilities. Quest runbooks are primarily ordered procedures: make each step actionable, state meaningful branches and approval gates, and give stages observable completion criteria. Vary the number of steps to fit the work. Add a context paragraph or sections for cross-cutting rules, output, or overall completion only when they improve the runbook; keep actions such as recording or publishing in the ordered workflow. Include the type's output and handoff, but follow the issue-tracker instructions for tracker operations. Keep the general instruction to use `/create-quest-type` in `skills/quest/SKILL.md`. `list.sh` reads each type's name and description from frontmatter, so avoid duplicating per-type registry details there. Done when the runbook is complete and the general instruction is correct.
